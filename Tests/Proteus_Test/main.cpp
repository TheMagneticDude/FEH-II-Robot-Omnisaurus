...
/* Global variable to keep track of being initialized to RCS */
int RCS_init = 0;
...
int MNMenu()
{
    ...
    FEHIcon::Icon MAIN[6];
    char main_label[8][20] = {"DC", "Servo", "Digital In", "Analog In", "Accel", "Touch", "Digital Out", "RCS"};
...
int RPMenu()
{
    ...
    if (!RCS_init)
    {
        RCS.InitializeTouchMenu("abcdefghi");
        RCS_init = 1;
    }

    LCD.Clear(BLACK);

    /* Create RCS menu icons */
    FEHIcon::Icon RP_T[1];
    char rp_t_label[1][20] = {"RCS"};
