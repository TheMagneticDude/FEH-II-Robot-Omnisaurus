...
FinalButton::FinalButton(HolonomicTriangleDrive &dt) : drivetrain(dt){
...
void FinalButton::init(){
    startTime = TimeNowMSec();
}

//Runs the command every tick
void FinalButton::run(){
    ...
    if(timeUp(startTime,totalDuration/2)){
        drivetrain.setMovementVector(0.13,0.5,0);
    }
    drivetrain.update();
